...
**Главное наблюдение:** при concurrency=10 Dynamic Batching не даёт ускорения ни на одной платформе. На GPU `no_batch` даже выигрывает у `medium` на 22%, потому что инференс ResNet50 на 4070 Ti настолько быстрый (единицы мс), что задержка очереди в 100 мс становится доминирующим расходом.
